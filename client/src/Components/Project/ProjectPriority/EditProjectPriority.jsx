...
const EditProjectPriority = () => {
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const { id } = useParams();
  const navigate = useNavigate();
  const { validToken, team, isLoading } = useAuth();
  const fieldPermissions = team?.role?.permissions?.projectPriority?.fields;
  const permissions = team?.role?.permissions?.projectPriority;

  const fetchSingleData = async (id) => {
    try {
      const response = await axios.get(`/api/v1/projectPriority/single-projectPriority/${id}`, {
        headers: {
      ...
      if (response?.data?.success) {
        setName(response?.data?.projectPriority?.name);
        setDescription(response?.data?.projectPriority?.description);
      };
    } catch (error) {
...
export default EditProjectPriority;